...
class SiteSettings(models.Model):
    ...
    booking_break_enabled = models.BooleanField(
        "הוסף הפסקה של רבע שעה אחרי הזמנות ארוכות (מעל 30 דקות)",
        default=True,
        help_text=(
            "כשהתיבה מסומנת — אחרי כל הזמנה שאורכת יותר מ-30 דקות, המערכת שומרת "
            "אוטומטית רבע שעה נוספת כהפסקה שבה אי אפשר להזמין תור. "
            "כשהתיבה לא מסומנת — לא תישמר הפסקה, וניתן יהיה להזמין תור מיד "
            "בסיום ההזמנה הקודמת."
        ),
    )
